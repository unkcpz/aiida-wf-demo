...
``aiida-wf-demo`` comes with a ``.github`` folder that contains continuous integration tests on every commit using `GitHub Actions <https://github.com/features/actions>`_. It will:
...
Once you're happy with your documentation, it's easy to host it online on ReadTheDocs_:
 ...
 #. Import your ``aiida-wf-demo`` repository (preferably using ``aiida-wf-demo`` as the project name)

The documentation is now available at `aiida-wf-demo.readthedocs.io <http://aiida-wf-demo.readthedocs.io/>`_.
